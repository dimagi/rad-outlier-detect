...
1. Make sure you have access to a form submission file from a CommCare project, along with its form summary and annotations. If you have access to the RAD server (10.0.2.160), use the *asha_func_checklist.gzip* file located in *data/forms/sangini*. 
    1. The form summary is located in the same folder with the same file name, except in *.csv* format.
    2. Annotations are located in *data/annotations/* as *sangini.csv*.
